<head>
<link href="css/Inconsolata.css" rel="stylesheet">
<link href="css/sprawl.css" rel="stylesheet">
<link id="page_favicon" href="data:image/x-icon;base64," rel="icon" type="image/x-icon">

<style>
body {
width: 60em;
}
</style>

<title>Countdown Tracker</title>
</head>
<body>

<script>
  (function(i,s,o,g,r,a,m){i['GoogleAnalyticsObject']=r;i[r]=i[r]||function(){
  (i[r].q=i[r].q||[]).push(arguments)},i[r].l=1*new Date();a=s.createElement(o),
  m=s.getElementsByTagName(o)[0];a.async=1;a.src=g;m.parentNode.insertBefore(a,m)
  })(window,document,'script','https://www.google-analytics.com/analytics.js','ga');

  ga('create', 'UA-1395087-7', 'auto');
  ga('send', 'pageview');
</script>

<h2>Welcome to The Countdown Tracker</h2>

<p>The Countdown Tracker is a web app created to allow MCs of <a href="https://en.wikipedia.org/wiki/Powered_by_the_Apocalypse">Powered by the Apocalypse</a> 
games to to track and share Countdown Clocks.  It was inspired by watching MCs freehand scribble Countdown Clocks on Google Hangouts and thinking that there 
needed to be another way. 

<p>The Tracker comes in two varieties.  A digital version modeled after the Countdown Clocks in  
<a href="http://www.drivethrurpg.com/product/171286/The-Sprawl----MIDNIGHT">The Sprawl</a>, and an analog 
version modeled aver the ones that appear in the rest of the <a href="https://en.wikipedia.org/wiki/Powered_by_the_Apocalypse">Powered by the Apocalypse</a> games.

 <h3>How it works:</h3>

<p>The MC clicks one of the <b>Create...</b> buttons below to create a new Countdown Controller. 

<p>They then share the link to The Big Board with their players (via Email, TXT, SMS, etc), and use the
Countdown Controller screen to manage their game's Countdown Clocks.  Modifications made to the clocks 
are instantly transmitted to the Big Board through the magic of <a href="http://lmgtfy.com/?q=websockets">WebSockets</a>.  

<h3>Important Note:</h3>

<p>Your Countdown Contoller it is identified by a unique URL.  That URL identifies you to the database as 
the owner of your Clocks.  You are the <i>only</i> person in the universe that knows it.  If you lose it, 
it's gone forever.  There is no way to get it back. So Bookmark it, email it to yourself, write it on a PostIt 
and stick it on your monitor.  Do whatever it takes to keep track of it.  

<p>(Actually, if you lose it, its no big deal. You can just make a new one). 

<h3>Chromecast</h3>
The MC screen has Google Cast support baked in, so if you have a Chromecast available you can cast away!  
If a Chromecast receiver is detected, the Google Cast icon will appear in the upper left hand corner of the page .  If one is detected 
the icon will appear .  If no Chromecast is detected, the 
icon will not appear.  Click the icon to connect to the Chromecast.

<h2>Create a new Countdown Tracker:</h2>
<form action="create.jsp">
  <input type="submit" class="button" style="background-color:green; color:black; font-weight: bold" value="Create Digital!">
</form>

<form action="analog-create.jsp">
  <input type="submit" class="button" style="background-color:AntiqueWhite ; color:black; font-weight: bold" value="Create Analog!">
</form>

<hr>

<h3>Affiliations</h3>
<p>This is just a just hobby page.
<p>It is in no way affiliated with any <a href="https://en.wikipedia.org/wiki/Powered_by_the_Apocalypse">Powered by the Apocalypse</a> games,
or <a href="http://www.drivethrurpg.com/product/171286/The-Sprawl----MIDNIGHT">The Sprawl</a>, 
or <a href="https://plus.google.com/117694340632268764245">Hamish Cameron </a> 
or <a href="http://www.ardens.org/">Ardens Ludere</a> 
or <a href="http://apocalypse-world.com/">Apocalypse World</a>
or D. Vincent Baker
or <a href = "http://lumpley.com">Lumpley Press</a>

or anyone else. 

<h3>Browser and Device support</h3>
I tested these pages on the latest versions of Chrome, Firefox, MS Edge, and Safari.  Ti does not work in IE.  I haven't tried it in Opera.  
It looks good on a 5" Android cell phone.  It's a bit smallish on an iPhone 4S. 

<div style="padding-top: 10em;">
<h3>Boring stuff</h3>
<h4>Data Retention</h4>
<p>All data is stored in a database on a server on the Internet and will be saved for 
future generations to enjoy... or until the server dies... or until the database 
is filled up... or someone figures out how to use this page for evil....  Until one of those
things happes, your data is as safe as the Rehdhat Openshift server it's stored on.

<h4>OpenShift</h4>
<p>This app is hosted on Bronze Tier <a href="https://www.openshift.com">OpenShift</a> 
server.  <a href="https://www.openshift.com">Openshift</a> <i>seems</i> pretty good, but 
I've only got a few weeks of experience with it so can't <i>really</i> vouch for it yet, 
but so far I like it.

<h4>Cookie Use</h4>
<p>Aside from google analytics, this page does not directly use cookies.
  
<h4>Technologies Involved</h4>
<p>The following Technologies were used during the creation of these pages
<ul>
<li>Openshift</li>
<li>Apache Tomcat 8</li>
<li>Orient DB</li>
<li>JSPs</li>
<li>WebSockets</li>
<li>jQuery</li>
<li>Knockout.js</li>
<li>Alertify.js</li>
<li>HTML</li>
<li>SVG</li>
<li>Eclipse Neon</li>
<li>Maven</li>
</ul>
</div>
</body>
</html>
